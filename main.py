...
def main():
    """
    Main function for collecting user data and generating the map.
    """
    ...
    USER_COUNTRY = locator.reverse(
        coordinates, language='en')[-2].split(", ")[-1]

    if USER_COUNTRY == 'United States':
        USER_COUNTRY = 'USA'

    df = pd.read_csv('locations.csv')
    df = df[(df['Country'] == USER_COUNTRY) & (df['Year'] == year)]
